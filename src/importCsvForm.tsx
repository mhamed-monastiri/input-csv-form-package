...
export default function ImportCsvDialog({
  ...
  exampleFile = [
    ...
    [dayjs().format('DD-MM-YYYY'), dayjs().format('DD-MM-YYYY'), 120, undefined, 'text'],
    [dayjs().format('DD-MM-YYYY'), dayjs().format('DD-MM-YYYY'), undefined, 500, 'text'],
    [dayjs().format('DD-MM-YYYY'), dayjs().format('DD-MM-YYYY'), 790, undefined, 'text'],
  ],
  ...other
}: Props) {
  ...
  const uploadEntriesSchema = Yup.object().shape({
    ...
    dateFormat: Yup.string().when(() =>
      existDate ? Yup.string().required('Date format is required') : Yup.string()
    ),
    ...
    columnsOrder: Yup.boolean().when(() =>
      existOrderFields ? Yup.array().required('Columns order is required') : Yup.array()
    ),
  });
  ...
  return (
    <DialogWithCloseButton fullWidth maxWidth="sm" open={open} onClose={handleClose} {...other}>
      <FormProvider methods={methods} onSubmit={handleSubmit(handleImport)}>
        <DialogTitle sx={{ p: (theme) => theme.spacing(3, 3, 2, 3) }}>Import CSV</DialogTitle>
        <DialogContent dividers sx={{ pt: 1, pb: 0, border: 'none' }}>
          <Box gap={2} display="grid" gridTemplateColumns={{ xs: 'repeat(1, 1fr)' }}>
            <Upload
              accept={{ 'file/csv': ['.csv'] }}
              multiple
            ...
            {values?.file && (
              <>
                ...
                >
                  <RHFSelect name="delimiterType" label="Delimiter *" placeholder="Delimiter">
                    ...
                    {[...delimiters, { label: 'Other', value: 'OTHER' }]?.map(
                      (delimiter, index) => (
                        ...
                        >
                          ...
                          {delimiter?.value !== 'OTHER' && <span>( {delimiter?.value} )</span>}
                        </MenuItem>
                      )
                    )}
                  </RHFSelect>
                ...
                >
                  <RHFSelect name="qualifierType" label="Qualifier *" placeholder="Qualifier">
                    <MenuItem value="" sx={{ fontStyle: 'italic', color: 'text.secondary' }}>
                      None
                    </MenuItem>
                    <Divider />
                    {[...qualifiers, { label: 'Other', value: 'OTHER' }]?.map(
                      (qualifier, index) => (
                        ...
                        >
                          ...
                          {qualifier?.value !== 'OTHER' && <span> {qualifier?.value} </span>}
                        </MenuItem>
                      )
                    )}
                  </RHFSelect>
                ...
                {existDate && (
                  <RHFSelect name="dateFormat" label="Date Format *" placeholder="Date Format">
                    <MenuItem value="" sx={{ fontStyle: 'italic', color: 'text.secondary' }}>
                      None
                    </MenuItem>
                    <Divider />
